...
struct t_mmlparser
{
    ...
    string mml;
    string cmdname;
    vector<t_param> param;
    const   CHAR*   cur;// change the pointer yet keep the content

    string& mp_get_cmd_name(){ return cmdname; };
    string& mp_get_param_name(UINT16D index){ return param[index].paramname; };
    CHAR*   mp_get_param_value(UINT8D index);

    VOID    mp_parse_topo(const CHAR* mml);
    VOID    mp_parse_topo_name();
    VOID    mp_parse_param();
    VOID    mp_parse_param_name(const UINT16D index);

    VOID    mp_parse_clear(){ param.clear(); };
};


typedef struct  tagLUT_PARAM
{
    FLT32   area;
    FLT32   cap;
    FLT32   resist;
    FLT32   offset;
    FLT32   leakpower;
    tagLUT_PARAM() : area(0), cap(0),resist(0),offset(0), leakpower(0) {};
}LUT_PARAM;
typedef vector<vector<LUT_PARAM> >   LIB_LUT;

VOID    begin_read_cell_param (istream &is, LIB_LUT& cell);

/* Add by Yiren, for the new parser*/
class wire_par {
public:
	wire_par(){};
	//wire_par(bool t){found = t;};
	~wire_par(){};
	string name; //wire name
	vector<int> out; // input vertex no.
	vector<int> in; // output vertexes no.
	bool is_primaryIn;
	bool is_primaryOut;
